...
TEST(CircleTest, SquareGettingFormula) { 
    ...
    EXPECT_NEAR(3.1415 * circle.getRadius() * circle.getRadius(), circle.getArea(), 1e-10);
}
...
TEST(CircleTest, MathematicalProperties) {
    ...
    EXPECT_NEAR(circle.getArea() / (circle.getRadius() * circle.getRadius()), 3.1415, 1e-10);
    
    // Квадрат длины окружности должен быть пропорционален площади
    // C² = 4πS
    EXPECT_NEAR(circle.getFerence() * circle.getFerence(), 4 * 3.1415 * circle.getArea(), 1e-8);
}
